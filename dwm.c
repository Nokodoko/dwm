...
/* ipc.h must come before config.h so IPCCommand type is defined */
#include "ipc.h"
...
/* pertag struct - stores per-tag layout settings */
struct Pertag {
	unsigned int curtag, prevtag; /* current and previous tag */
	int nmasters[LENGTH(tags) + 1]; /* number of clients in master area */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	/* Filter out control characters used for block signals */
	i = -1, j = 0;
	while (stext[++i])
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if (c->tags & SCRATCHPAD_TAG) {
		...
		c->x = selmon->wx;
		c->y = selmon->wy;
		c->w = 53 * 9; /* 53 columns * ~9px per char (monospace) */
		c->h = selmon->wh;
		resizeclient(c, c->x, c->y, c->w, c->h);
	} else if (c->tags & AI_SCRATCHPAD_TAG) {
		c->w = 53 * 9;
		c->x = selmon->wx + selmon->ww - c->w;
		c->y = selmon->wy;
		c->h = selmon->wh;
		resizeclient(c, c->x, c->y, c->w, c->h);
	} else if (c->tags & STEAM_SCRATCHPAD_TAG) {
		c->w = 53 * 9;
		c->x = selmon->wx + selmon->ww - c->w;
		c->y = selmon->wy;
		c->h = selmon->wh;
	...
	/* Ensure scratchpads get focus after positioning (counteract enternotify) */
...
unfocus(Client *c, int setfocus)
{
	if (!c)
		return;
	grabbuttons(c, 0);
	XSetWindowBorder(dpy, c->win, scheme[c->borderscheme >= 0 ? c->borderscheme : SchemeNorm][ColBorder].pixel);
	if (setfocus) {
		XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
		XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
	}
	...
	if (XineramaIsActive(dpy)) {
		...
		for (i = 0, m = mons; i < nn && m; m = m->next, i++)
			...
			|| unique[i].width != m->mw || unique[i].height != m->mh)
			{
				dirty = 1;
				m->num = i;
				if (i < LENGTH(defaulttags))
					m->tagset[0] = m->tagset[1] = defaulttags[i];
...
togglescratch(const Arg *arg)
{
	...
	if (!c) {
		...
		selmon->scratchvisible |= scratchtag;
		spawn(arg);
		return;
	}

	/* Toggle visibility via scratchvisible (does not modify tagset) */
	if (selmon->scratchvisible & scratchtag) {
		/* Currently visible on this monitor, hide it */
		selmon->scratchvisible &= ~scratchtag;
		arrange(selmon);
		return;
	} else {
		...
		/* Move scratchpad to current monitor if on another */
		if (c->mon != selmon) {
		...
		if (scratchtag == SCRATCHPAD_TAG) {
			...
			c->x = selmon->wx + selmon->ww - c->w;
			c->y = selmon->wy;
		} else if (scratchtag == STEAM_SCRATCHPAD_TAG) {
			c->w = 53 * 9;
			c->h = selmon->wh;
			c->x = selmon->wx + selmon->ww - c->w;
			c->y = selmon->wy;
